...
/**
 * ISLAND MARKER — the adventure map's pin, drawn as a tiny illustrated
 * island instead of a plain dot. `live` gets a warmer, fuller treatment
 * (palm + sand); `planned` is a quieter grey-green silhouette, still
 * clearly an island (so the map reads as "more of the world," not
 * "disabled buttons").
 */
export function islandMarkerSVG(status, size = 46){
  const isLive = status === "live" || status === "in-production";
  const land = isLive ? "#D9C08A" : "#C9CFC6";
  const foliage = isLive ? "#7C9885" : "#B7BFB6";
  const trunk = isLive ? "#8B6B43" : "#A9ABA1";
  return `
  <svg width="${size}" height="${size}" viewBox="0 0 64 64" role="img" aria-hidden="true">
    <ellipse cx="32" cy="46" rx="24" ry="8" fill="${land}" opacity=".55"/>
    <path d="M14 44 C14 30 22 20 32 20 C42 20 50 30 50 44 Z" fill="${land}"/>
    <circle cx="27" cy="30" r="9" fill="${foliage}"/>
    <circle cx="35" cy="26" r="7" fill="${foliage}"/>
    <rect x="30" y="30" width="3" height="14" rx="1.5" fill="${trunk}"/>
  </svg>`;
}
